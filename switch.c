...
double
intended_run(struct proc_type *pc)
{
	return -1 * pc->mean_cpu_time * log(drand());
}

int
should_unlock(struct proc_type *pc)
{
	return drand() < pc->unblocking_probability;
}

void
init_simulation(int steps)
{
...
step()
{
	...
	for (int i = 0; i < NUMPROC; i++) {
		if (proc[i].status == BLOCKED &&
		    should_unlock(&sim_status.proc_type[i])) {
...
print_stats()
{
	...
	for (int i = 0; i < NUMPROC; i++) {
		printf("Proc %d: %.0f%%\n",
		       i,
		       100 * (double) sim_status.proc_stats[i].runtime / rt);
	}
}